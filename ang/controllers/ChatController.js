...
angular.module('myApp').controller('ChatController', ['$scope', '$rootScope', 'AjaxHelper', 'SocketService', 'AuthService', '$location', '$timeout', function($scope, $rootScope, AjaxHelper, SocketService, AuthService, $location, $timeout) {
    ...
    function updateGroupConversationData(groupId, message, timestamp, senderName) {
        ...
        $scope.groupConversationData[groupId].lastMessageTime = new Date(timestamp);
        $scope.groupConversationData[groupId].timestamp = new Date(timestamp).getTime();
        $scope.groupConversationData[groupId].senderName = senderName;
        
        console.log('Updated group conversation data for', groupId, ':', $scope.groupConversationData[groupId]);
    }
    ...
    function filterConversations() {
        if (!$scope.searchQuery) {
            $scope.filteredConversations = $scope.conversationStudents.filter(function(item) {
            ...
            $scope.filteredConversations = $scope.conversationStudents.filter(function(item) {
                ...
                if (item.group_id) {
                    return item.name.toLowerCase().includes($scope.searchQuery.toLowerCase()) ||
                           (item.description && item.description.toLowerCase().includes($scope.searchQuery.toLowerCase()));
                }
                
                if (!item.email || !item.name) return false;
                return item.name.toLowerCase().includes($scope.searchQuery.toLowerCase()) ||
                       item.email.toLowerCase().includes($scope.searchQuery.toLowerCase());
            });
        }
    }
    ...
    function loadConversationSummary() {
        ...
        AjaxHelper.ajaxRequest('GET', '/auth/get_last_messages_summary')
            .then(function(response) {
                ...
                console.log('Found', $scope.conversationStudents.length, 'total conversations');
    ...
    function loadGroupMessages() {
        ...
        AjaxHelper.ajaxRequest('GET', '/auth/get_messages', { group_id: $scope.selectedGroup.group_id })
            .then(function(response) {
            ...
            .catch(function(error) {
                ...
                $scope.messages = [];
                $scope.flashMessage = error.flashMessage || 'Failed to load group messages';
                $scope.flashType = error.flashType || 'error';
                $rootScope.$emit('flashMessage', { 
                    message: $scope.flashMessage, 
                    type: $scope.flashType 
    ...
    $scope.sendMessage = function() {
        if (!$scope.newMessage || (!$scope.selectedStudentEmail && !$scope.selectedGroup)) {
            $scope.flashMessage = 'Please enter a message and select a conversation';
            $scope.flashType = 'error';
            $rootScope.$emit('flashMessage', { 
                message: $scope.flashMessage, 
                type: $scope.flashType 
        ...
        if ($scope.chatType === 'group') {
            ...
            updateGroupConversationData($scope.selectedGroup.group_id, $scope.newMessage, new Date(), 'You');
            sortConversationsByLastMessage();
    ...
    $scope.openCreateGroupModal = function() {
        ...
        try {
            ...
            console.log('Modal opened successfully, showCreateGroupModal:', $scope.showCreateGroupModal);
    ...
    $scope.closeCreateGroupModal = function() {
        ...
        try {
            ...
            console.log('Modal closed successfully, showCreateGroupModal:', $scope.showCreateGroupModal);
    ...
    $scope.toggleMemberSelection = function(student) {
        ...
        console.log('toggleMemberSelection: selectedMembers after toggle:', $scope.selectedMembers);
    };
    ...
    SocketService.on('group_message', function(data) {
        ...
        updateGroupConversationData(data.group_id, data.message, data.created_at, data.sender_name);
        
        if ($scope.selectedGroup && data.group_id == $scope.selectedGroup.group_id) {
            $scope.messages.push(data);
            scrollToBottom();
        }
    ...
    SocketService.on('group_created', function(data) {
        ...
        if ($scope.senderEmail === data.created_by || $scope.selectedMembers.includes($scope.senderEmail)) {
            var newGroup = {
