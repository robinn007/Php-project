...
angular.module("myApp").controller("ChatController", [
  ...
  function (
    ...
    $scope.memberSearchQuery = ""; // Added for member search in modal
    ...
    $scope.flashMessage = "";
    $scope.flashType = "";
    $scope.typingUsers = {}; // { email: true/false } or { group_id: { email: name } }
    ...
    // FIXED MODAL FUNCTIONS - These are the key fixes
    $scope.openCreateGroupModal = function () {
      console.log("openCreateGroupModal called at", new Date().toISOString());
      try {
        // Reset all modal-related variables
        $scope.showCreateGroupModal = true;
        $scope.newGroupName = "";
        $scope.newGroupDescription = "";
        $scope.selectedMembers = [];
        $scope.memberSearchQuery = "";
        $scope.flashMessage = "";
        $scope.flashType = "";

        // Initialize students selection state
        initializeStudents();

        console.log(
          "Modal opened successfully, showCreateGroupModal:",
          $scope.showCreateGroupModal
        );

        // Use $scope.$apply() instead of $applyAsync() for immediate UI update
        if (!$scope.$phase) {
          $scope.$apply();
        }
      } catch (error) {
    ...
    // FIXED CLOSE MODAL FUNCTION - This is the main fix for your issue
    $scope.closeCreateGroupModal = function () {
      console.log("closeCreateGroupModal called at", new Date().toISOString());
      try {
        ...
        console.log(
          "Modal closed successfully, showCreateGroupModal:",
          $scope.showCreateGroupModal
        );

        // Force immediate digest cycle
        if (!$scope.$phase) {
          $scope.$apply();
        }
      } catch (error) {
        console.error("Error in closeCreateGroupModal:", error);
    ...
    $scope.toggleMember = function (email) {
      ...
      // Update the student's selected state
      $scope.allStudents = $scope.allStudents.map(function (student) {
        student.selected = $scope.selectedMembers.includes(student.email);
        return student;
      });
      ...
      console.log(
        "toggleMember: Current selectedMembers:",
        $scope.selectedMembers
      );
    };
    ...
    $scope.toggleMemberSelection = function (student) {
      ...
      console.log(
        "toggleMemberSelection: selectedMembers after toggle:",
        $scope.selectedMembers
      );
    };
    ...
    $scope.stopTyping = function () {
      ...
      } else if ($scope.selectedStudentEmail) {
        SocketService.emit("typing_stop", {
          sender_email: $scope.senderEmail,
          receiver_email: $scope.selectedStudentEmail,
          type: "direct",
    ...
    // Get typing indicator text for direct chat
    $scope.getTypingIndicator = function () {
      if ($scope.chatType === "direct" && $scope.selectedStudentEmail) {
        return $scope.typingUsers[$scope.selectedStudentEmail]
          ? "typing..."
          : "";
      }
      return "";
    ...
    // Get typing indicator text for group chat
    $scope.getGroupTypingIndicator = function () {
      if ($scope.chatType === "group" && $scope.selectedGroup) {
        var groupTyping = $scope.typingUsers[$scope.selectedGroup.group_id];
        if (groupTyping && Object.keys(groupTyping).length > 0) {
          var names = Object.values(groupTyping);
          if (names.length === 1) {
    ...
    // Update the sendMessage function to stop typing
    var originalSendMessage = $scope.sendMessage;
    $scope.sendMessage = function () {
      $scope.stopTyping();
      originalSendMessage();
    };
    ...
    SocketService.on("typing_start", function (data) {
      console.log("Received typing_start:", data);
      ...
      if (
        ...
        // Initialize group typing object if needed
        if (!$scope.typingUsers[$scope.selectedGroup.group_id]) {
          $scope.typingUsers[$scope.selectedGroup.group_id] = {};
        }
        $scope.typingUsers[$scope.selectedGroup.group_id][data.sender_email] =
          data.sender_name;
      } else if (
        data.type === "direct" &&
        data.sender_email === $scope.selectedStudentEmail
      ) {
        $scope.typingUsers[data.sender_email] = true;
      }

      $scope.$applyAsync();
    });

    SocketService.on("typing_stop", function (data) {
      console.log("Received typing_stop:", data);
      ...
      if (
        ...
        if ($scope.typingUsers[$scope.selectedGroup.group_id]) {
          delete $scope.typingUsers[$scope.selectedGroup.group_id][
            data.sender_email
          ];

          // Clean up empty object
          if (
            Object.keys($scope.typingUsers[$scope.selectedGroup.group_id])
              .length === 0
          ) {
            delete $scope.typingUsers[$scope.selectedGroup.group_id];
          }
        ...
        delete $scope.typingUsers[data.sender_email];
      }

      $scope.$applyAsync();
    });

    // Modify the autoResize function to include typing detection
    $scope.autoResize = function () {
      $scope.handleTyping();
    ...
    var originalSelectStudent = $scope.selectStudent;
    $scope.selectStudent = function (student) {
      $scope.stopTyping();
      $scope.typingUsers = {};
      originalSelectStudent(student);
    };

    var originalSelectGroup = $scope.selectGroup;
    $scope.selectGroup = function (group) {
      $scope.stopTyping();
      $scope.typingUsers = {};
